...
class PlotMaster(object):
    ...
    def plot_clustermap(self):
        ...
        sum_list = [self.bag_cont.col_exp, self.bag_cont.col_bio_rep, self.bag_cont.col_tech_rep] # self.bag_cont.col_charge self.bag_cont.col_weight_type
        ...
        df = self.bag_cont.get_pivot(sum_list, mean_list, [self.bag_cont.col_level])
        ...
        df, dist_orig, dist_imputed = self.bag_cont.log2_and_fillna_with_normal_dist(df)
    ...
    def plot_bar(self):
        ...
        ax = sns.barplot(x=self.bag_cont.col_level, y=self.bag_cont.col_area_sum_total, hue=self.bag_cont.col_exp, data=df)
    ...
    def plot_ms1_area_std(self):
        sum_list = [self.bag_cont.col_exp, self.bag_cont.col_bio_rep,self.bag_cont.col_link_type, self.bag_cont.col_tech_rep]
        ...
        df_pval = self.bag_cont.get_two_sided_ttest(pval_list, [self.bag_cont.col_exp, self.bag_cont.col_bio_rep])
        df = self.bag_cont.get_stats(sum_list, mean_list)
        df = pd.merge(df, df_pval, on=self.bag_cont.col_level)
        print(df)
        fg = sns.relplot(data=df, x='mean', y='std', col=self.bag_cont.col_exp, hue=self.bag_cont.col_link_type)
        self.plot_fig(name="std")
        # df = df[(df[self.bag_cont.col_log2ratio] < 1)&(df[self.bag_cont.col_log2ratio] > -1)]
        print(df)
        fg = sns.relplot(data=df, y='qval', x='log2ratio', col=self.bag_cont.col_exp, hue='std', palette="brg")
        for row in fg.axes:
            for ax in row:
                ax.hlines(y=0.05, xmin=df[self.bag_cont.col_log2ratio].min(), xmax=df[self.bag_cont.col_log2ratio].max(), label="qval=0.05", colors=['purple'])
        self.plot_fig(name="ttest")
    ...
    def plot_fig(self, name, extra=''):
        filter = ""
        if self.bag_cont.filter:
            filter = '_' + self.bag_cont.filter
        if extra:
            extra = '_' + extra
        plib.save_fig("bag_{0}_{1}{2}{3}".format(name, self.bag_cont.col_level, filter, extra), self.out_folder)